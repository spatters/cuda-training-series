...
__global__ void row_sums(const float *A, float *sums, size_t ds){
  ...
  if (idx < ds){
    ...
    for (size_t i = 0; i < ds; i++)
      sum += A[ds * idx + i];         // write a for loop that will cause the thread to iterate across a row, keeeping a running sum, and write the result to sums
    sums[idx] = sum;
}}
...
int main(){
  ...
  cudaMalloc(&d_sums, DSIZE*sizeof(float));  // allocate device space for d_sums
